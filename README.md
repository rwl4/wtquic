...
- **Spec**: draft-ietf-webtrans-http3-15 (plus legacy compatibility on
  receive for older draft SETTINGS codepoints).
...
draft-ietf-webtrans-http3-15 requires the RESET_STREAM_AT QUIC extension
...
> **Both backends are draft-15 WebTransport-compatible in *lenient
...
- **Callback confinement**: callback dispatch and every session/stream
  ...
  entered from outside a callback is backend-defined. On MsQuic there
  is no out-of-callback entry: operate only from that session's
  callbacks, except that a retained session may be inspected and
  released from any thread once `wtq_msquic_env_close` has returned.
  Blocking teardown (`env_close`, `listener_stop`) must not be called
  from a callback. On Network.framework, `wtq_nw_conn_post` is the
