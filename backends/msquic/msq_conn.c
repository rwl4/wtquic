                                    ...
                                    bool is_client)
{
     ...
     * The driver is allocated here once and never copied. Every connection
...
{
    /* Precondition: no session is attached (never published) — the caller's
     * failure paths detach it first. Mark abandoned so the SHUTDOWN_COMPLETE
     * that ConnectionClose (inside wtq_msq_conn_free) delivers synchronously is
     * ignored without entering the guard and, crucially, does NOT re-free the
     * driver. This call is the single owner of the free. */
    atomic_store_explicit(&drv->abandon, true, memory_order_release);
...
const wtq_driver_ops_t *wtq_msq_driver_ops(void)
{
    static const wtq_driver_ops_t ops = {
         ...
         * redelivery and does NOT extend receive credit while nothing is
         * consumed), so a paused peer is eventually blocked by QUIC flow
...
/* The event switch, run inside the guard bracket by wtq_msq_conn_callback.
 * May free `drv` (SHUTDOWN_COMPLETE) — the wrapper reads the guard's
 * leave/ctx into locals before calling this. */
static QUIC_STATUS conn_dispatch(HQUIC conn, struct wtq_driver *drv,
    ...
    case QUIC_CONNECTION_EVENT_SHUTDOWN_COMPLETE:
        ...
        if (drv->session != NULL) {
            ...
            wtq_conn_on_conn_closed(wtq_api_session_conn(drv->session),
                                    ...
                                    wtq_msq_now_us());
            /* Transport quiescence (managed hook): the last look at a
             * still-valid session handle, under the guard, before the
             * backend drops its own reference. Marks only — must not
             * release the app's reference. Fires exactly once here, only
             * for a connection that ran (an abandoned accept never reaches
             * a callback). */
                                           ...
                                           QUIC_CONNECTION_EVENT *ev)
{
    ...
    /* An abandoned connection (opened but never started/published) has no
     * attached session and emits no events; its wtq_msq_conn_abandon() caller
     * owns the free. Ignore the synchronous close-time SHUTDOWN_COMPLETE here
     ...
     * dispatch: SHUTDOWN_COMPLETE frees drv, so drv->guard is unreadable
     * afterward. enter/leave are both NULL for an unguarded (legacy)
     * connection. */
