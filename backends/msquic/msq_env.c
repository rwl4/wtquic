                                 ...
                                 wtq_msquic_env_t **env_out)
{
    ...
    *env_out = NULL;
    if (cfg_in == NULL)
        return WTQ_ERR_INVALID_ARG;
    /* Require the complete pointer-bearing prefix: alloc / existing_api /
     * existing_registration / app_name are dereferenced or stored below, so
     * a struct_size landing mid-pointer must be rejected, not copied as a
     * truncated address. (tuning past it may be absent -> defaults.) */
    if (!wtq_cfg_has(cfg_in->struct_size,
                     offsetof(wtq_msquic_env_cfg_t, app_name),
                     sizeof(((wtq_msquic_env_cfg_t *)0)->app_name)))
        return WTQ_ERR_INVALID_ARG;

    cfg_copy(&cfg, sizeof(cfg), cfg_in, cfg_in->struct_size);
    /* An outer struct_size that only PARTIALLY covers the nested tuning would
     * leave tuning.struct_size nonzero (from the copied bytes) and skip the
     * defaulting below. Treat a not-wholly-present tuning as absent. */
    if (!wtq_cfg_has(cfg_in->struct_size,
                     offsetof(wtq_msquic_env_cfg_t, tuning), sizeof(cfg.tuning)))
        memset(&cfg.tuning, 0, sizeof(cfg.tuning));
