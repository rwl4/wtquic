/*
 * EXPERIMENTAL raw MsQuic observer peer — NOT a backend, NOT production.
 *
 * A bare QUIC server that reports what actually crosses the wire, so the
 * Network.framework probe's client-side API calls are validated by peer
 ...
 */
#ifndef WTQ_NW_RAW_PEER_H
#define WTQ_NW_RAW_PEER_H
