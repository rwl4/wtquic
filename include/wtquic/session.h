 ...
 *   provides. The MsQuic backend (<wtquic/wtquic_msquic.h>) provides
 *   no out-of-callback entry at all: operations happen only inside
 *   that session's callbacks, with one documented exception after
 *   wtq_msquic_env_close() returns.
 */
 ...
 * subprotocols to offer). Strings are copied during the call. Use
 * WTQ_CONNECT_CONFIG_INIT / wtq_connect_config_init.
 */
...
#define WTQ_CONNECT_CONFIG_INIT \
    { (uint32_t)sizeof(wtq_connect_config_t), 0, 0, 0, 0, 0, 0 }
