 ...
 * session gets exactly one start attempt. A failure returns the driver's
 * error and leaves wire-visible state that cannot be undone: release the
 * session and build a new connection rather than retrying. Every later
 * call returns WTQ_ERR_STATE and runs no driver operation. */
