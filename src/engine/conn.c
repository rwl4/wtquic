                             ...
                             const wtq_driver_ops_t *ops, wtq_conn_t **out)
{
    ...
    conn->legacy = cfg->send_legacy_wt;
...
wtq_result_t wtq_conn_start(wtq_conn_t *conn, uint64_t now_us)
{
    ...
    uint8_t buf[1 + 8 + 64];
    wtq_h3_settings_encode_cfg_t scfg = { conn->ecp, conn->legacy };
    size_t flen = 0;
    ...
    /*
     * ONE-SHOT: the attempt is consumed before the first driver op.
     * A failure part-way leaves an opened stream (and possibly a
     * transmitted SETTINGS frame) on the wire, and neither can be
     * un-sent — so no retry is possible and no rollback is attempted.
     * Every later call returns WTQ_ERR_STATE, touching no driver op.
     */
    conn->started = true;
...
static void server_request_done(wtq_conn_t *conn, struct wtq_estream *es)
{
    ...
    wtq_connect_status_t st = wtq_connect_decode_request(
        es->set_buf, es->set_fill, &CONN_STRICT_OPTS, &req, offered,
                                     ...
                                     const wtq_client_connect_cfg_t *cfg)
{
    ...
    /* decode the request we just built, exactly as our server would */
    {
        ...
        wtq_connect_status_t dst = wtq_connect_decode_request(
            section, slen, &CONN_STRICT_OPTS, &rq, got,
    ...
    /* Committed: copy the offer into connection-owned storage (needed
     * later to validate the server's WT-Protocol pick). */
    buf_off = 0;
    for (size_t i = 0; i < cfg->protocol_count; i++) {
