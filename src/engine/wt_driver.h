...
typedef struct wtq_conn_cfg {
    ...
    bool send_legacy_wt;             /* outgoing legacy compat knob */
...
/*
 * Transport ready: opens the local control stream (type 0x00) and
 * sends the SETTINGS frame, then opens the QPACK encoder (0x02) and
 * decoder (0x03) streams with their one-byte prefaces (no dynamic
 * QPACK ever follows).
 *
 * ONE-SHOT. A connection gets exactly one start attempt, consumed
 * before the first driver operation runs. On failure the original
 * driver error is returned and NOTHING is rolled back: streams already
